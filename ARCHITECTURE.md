...
**Known gap, not yet fixed:** if the `PushQueue.create()` insert itself fails (DB hiccup, connection drop) in the narrow window right after the real mutation already committed, there is **no row at all** to retry — not even a `FAILED` one. It's invisible in `console.error` only, and (confirmed, now that 04-05's sync-status dashboard is live) invisible there too, since that page only surfaces existing `PushQueue` rows. Narrow window, but a real silent-failure path per app-build-sop.md's checklist — worth a reconciliation job (compare `RatePlan.otaPrice`/recent bookings against `PushQueue` history) before scaling past a single pilot hotel, not blocking today. Also tracked in STATE.md's Deferred Issues table.

**04-05 (sync status API + dashboard badge + manual retry) — applied 2026-08-24, committed `409aba9`. Phase 4 complete, 5/5 plans.** `GET/POST /api/sync-status[/:id/retry]` (`src/routes/syncStatus.ts`, mounted in `app.ts`), `frontend/src/pages/SyncStatus.tsx` + a red badge on `Dashboard.tsx` linking to it. Both routes gate on `ADMIN_ROLES` (`HOTEL_ADMIN`/`SUPER_ADMIN`) via `requireAuth`, and both scope every lookup `{ id, hotelId }` off the JWT claims — never a bare `{ id }` — same tenant-isolation pattern as `roomTypes.ts`/`bookings.ts`, including a byte-identical 404 body (`PUSH_QUEUE_NOT_FOUND_BODY`) for cross-tenant vs. genuinely-missing ids.

The audit caught and fixed two real gaps before any code was written, both confirmed live in the shipped code: (1) retry never touches `PushQueue.lastError` — it stays as the last-known failure reason until the next real push attempt overwrites it, so the evidence survives the exact moment a human intervenes; (2) retry is rate-limited with a 60s cooldown (`RETRY_COOLDOWN_MS`, checked against the new `lastRetriedAt` field, 409 if tripped) plus an accountability field (`retriedByUserId`) — so retry can't be looped to reset `attempts` to 0 forever and defeat the worker's own `MAX_ATTEMPTS` circuit breaker against Channex's rate-limited, eventually-paid API. New migration `20260824121740_add_push_queue_retry_fields`, applied and confirmed live against the real dev DB (`prisma migrate status`: up to date).

Verified via 23/23 automated API checks, 10/10 automated Playwright browser checks (both throwaway, not committed), and a live checkpoint the user personally clicked through (failure badge appeared, failed row showed a readable error and the resolved rate-plan name — not a raw cuid — retry worked, badge cleared after).

## Error + uptime monitoring — Sentry + UptimeRobot (Phase 7, Plan 07-02 — complete, UNIFY'd 2026-08-31, commit `5dabb59`)

Sentry wired into backend (`src/lib/sentry.ts`, `@sentry/node`) and frontend (`frontend/src/main.tsx`, `@sentry/react`), committed `38f866e`/`37062a3`, pushed, TypeScript compiles clean on both. Live-verified against real staging (not just configured): a real backend error and a real frontend error both captured in Sentry, AC-8 PII/auth-header scrubbing confirmed against the actual captured events (fake `Authorization` header produced zero Request/Headers data backend-side; frontend Fetch breadcrumbs showed only method/URL/status), both UptimeRobot monitors confirmed Up with a delivered test alert, `security-review` on `auth.ts`/`channexWebhook.ts` found zero HIGH/MEDIUM findings. Full results: `.paul/phases/07-pre-launch-gate/07-02-SUMMARY.md`. Key facts for future-Claude:

- **This codebase's routes never call `next(err)`** — every route catches internally and manually responds 500 (verified via grep). `Sentry.setupExpressErrorHandler(app)` in `app.ts` is a documented backstop only and will never see a real route error. The actual capture is `Sentry.captureException(err)` added directly inside each route file's own catch block (8 files: `auth`, `bookings`, `hotel`, `ratePlans`, `roomTypes`, `rooms`, `syncStatus`, `channexWebhook`). **Any new route added later needs the same explicit `Sentry.captureException(err)` call in its own catch block** — wiring only the global handler captures nothing in this codebase's error-handling shape.
- `pushQueueWorker.ts` has two independent catch sites (`processRow`'s own try/catch, and the outer tick's `.catch()`) — both wired to a shared 15-minute de-dupe-by-message helper (`captureWorkerError`), so a persistent failure at the 7s tick interval can't burn the Sentry free-tier quota (would otherwise be ~12,300 events/day unmitigated).
- `/health` now runs a real `SELECT 1` via Prisma, returns 503 on DB failure — was a static 200 before, which would have shown a false "Up" to uptime monitoring during a real DB outage.
- `sendDefaultPii: false` + explicit Authorization-header stripping on both SDKs (`beforeSend` backend, `beforeBreadcrumb` frontend) — deliberate, since `frontend/src/lib/api.ts` sets that header on every authenticated request; this is what keeps guest PII / staff JWTs out of the third-party SaaS.
- Backend Sentry init is fail-open (`src/lib/sentry.ts`): missing or malformed `SENTRY_DSN` logs and continues, never crashes server startup.
- **Vercel bakes `VITE_*` env vars in at build time, not runtime** — adding/changing `VITE_SENTRY_DSN` in the Vercel dashboard does nothing until a build actually runs after the change; caught by checking deployment timestamps against when the var was added, fixed with a manual redeploy. Any future Vite env-var change on this project needs the same check.
- **A directly-typed `throw` in Chrome DevTools console doesn't reliably trigger `window.onerror`/Sentry's global handler** — wrap it in `setTimeout(() => { throw new Error(...) }, 0)` when manually testing frontend error monitoring from DevTools; a bare console throw looked like a broken integration on the first two attempts before this was diagnosed.

**Known, not blocking:** a stray, unrelated Vercel project (pointed at the repo root instead of `frontend/`, failing every build since before this session) was surfaced during live verification — flagged for the user to delete at their convenience, not touched, not caused by this plan.
